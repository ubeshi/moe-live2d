  ...
  protected parseValue(
    ...
    for (; i < length; i++) {
      ...
      switch (c) {
        ...
        case '"':
          const string = this.parseString(buffer, length, i + 1, outEndPos);
          if (string === null) {
            throw new Error();
          }
          return new JsonString(string); // \"の次の文字から
        case '[':
...
export class JsonFloat extends Value {
   ...
   * Valueの種類が数値型ならtrue
   */
  public override isFloat(): boolean {
    return true;
  }
   ...
   * 要素を数値型で返す(number)
   */
  public override toInt(defaultValue = 0): number {
   ...
   * 要素を数値型で返す(number)
   */
  public override toFloat(defaultValue = 0.0): number {
    return this._value;
  }
   ...
   * 引数の値と等しければtrue
   */
  public override equals(value: csmString): boolean;
  public override equals(value: string): boolean;
  public override equals(value: number): boolean;
  public override equals(value: boolean): boolean;
  public override equals(value: any): boolean {
...
export class JsonBoolean extends Value {
   ...
   * Valueの種類が真偽値ならtrue
   */
  public override isBool(): boolean {
    return true;
  }
   ...
   * 要素を真偽値で返す(boolean)
   */
  public override toBoolean(defaultValue = false): boolean {
...
export class JsonError extends JsonString {
   ...
   * エラー情報をセットする
   */
  public override setErrorNotForClientCall(s: string): Value {
    this._stringBuffer = s;
    return this;
  }
...
export class JsonArray extends Value {
   ...
   * 添字演算子[index]
   */
  public override getValueByIndex(index: number): Value {
   ...
   * 添字演算子[string | csmString]
   */
  public override getValueByString(s: string | csmString): Value {
    return Value.errorValue.setErrorNotForClientCall(
      CSM_JSON_ERROR_TYPE_MISMATCH
    );
  }
   ...
   */
  public override getVector(defaultValue: csmVector<Value>): csmVector<Value> {
    return this._array;
  }
...
export class JsonMap extends Value {
   ...
   * 要素をMap型で返す
   */
  public override getMap(defaultValue?: csmMap<string, Value>): csmMap<string, Value> {
    return this._map;
  }
   ...
   * Mapからキーのリストを取得する
   */
  public override getKeys(): csmVector<string> {
    if (!this._keys) {
